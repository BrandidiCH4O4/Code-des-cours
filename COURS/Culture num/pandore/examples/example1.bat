...
rem @(#)example1.bat
rem 
rem (C)Régis Clouard - 2001-02-01
rem 

rem Verification du nombre d'arguments (ici 1 seul).
if "%1" == "" goto ERROR
if "%2" == "" goto ERROR

rem  Iteration de l'operateur moyenneur.
set in="%1"
pmeanfiltering 1 %1 im1.pan
pmeanfiltering 1 im1.pan im1.pan

rem Seuille l'image par variance interclasse.
pvariancebinarization im1.pan %2

call pstatus
call pset seuil
@echo Seuillage a la valeur : %seuil%

del pset.bat
goto END

:ERROR
@echo ERROR: usage: %0 image_in image_out
